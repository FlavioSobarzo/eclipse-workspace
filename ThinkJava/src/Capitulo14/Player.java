...
public class Player {
    ...
    public Card searchForMatch(Card prev) {
    	Card[] cartasPosibles = null;
    	int[] guardarNumCarta = new int[hand.size()];
    	int contador = 0;
        for (int i = 0; i < hand.size(); i++) {
            Card card = hand.getCard(i);
            if (cardMatches(card, prev)) {
                cartasPosibles[contador] = card;
                guardarNumCarta[contador] = i;
                contador = contador+1;
            }
        }
        if(cartasPosibles != null) {
        	for(int i =0; i<cartasPosibles.length; i++) {
	        	if(cartasPosibles[i].getRank() == 8) {
	        		return hand.popCard(guardarNumCarta[i]);
	        	}
	        }
        	int cartaMayor = 0;
        	for(int i =0; i<cartasPosibles.length; i++) {
        		if(cartasPosibles[cartaMayor].getRank() < cartasPosibles[i].getRank()) {
        			cartaMayor=i;
        		}
        	}
        	return hand.popCard(guardarNumCarta[cartaMayor]);
	    }
        return null;
